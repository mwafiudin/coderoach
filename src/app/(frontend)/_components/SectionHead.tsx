...
export function SectionHead({ marker, category, description, heading, lede, className = '' }: Props) {
  return (
    <div className={`reveal-stagger ${className}`}>
      <span className="font-mono text-xs font-medium tracking-wider text-mist-600 uppercase inline-flex items-center gap-2 flex-wrap tabular">
        {marker} <span className="text-mist-400">·</span> {category} <span className="text-mist-400">//</span>{' '}
        <span className="text-electric">{description}</span>
      </span>
